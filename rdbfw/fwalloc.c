...
int rdbfw_alloc_prealloc (uint32_t sz, uint32_t count, uint32_t max_count, int hint_sb_sz) {
    ...
    if ( sz > MAX_RDBFW_ALLOC_SZ || sz == 0 ) {
        fwlog (LOG_ERROR, "Sanity: Failed to prealloc memory - called with sz:%"PRIu32", ct: %"PRIu32
    ...
    if (am == NULL) {
        ...
        if ( sz > MAX_RDBFW_ALLOC_SZ ||
            ...
            fwlog (LOG_ERROR, "Failed to prealloc memory - called with sz:%"PRIu32", ct: %"PRIu32
                    ", max_ct %"PRIu32", pool = %p\n", sz, count, max_count, alloc_master_pool); 
            goto rdbfw_alloc_prealloc_err;
        }
        am = calloc (1,sizeof (alloc_master_t));
        if (am == NULL) {
            fwlog (LOG_ERROR, "calloc-error\n");
            goto rdbfw_alloc_prealloc_err;
        }
        ...
        fwlog (LOG_DEBUG, "sb_cnt = %d\tsb_sz = %d(g=%d)\tsb_item_cnt = %d\t block_sz = %d\t, gross_sz = %d\n",
        ...
        if ( am->superblock_pool == NULL) {
            fwlog ( LOG_ERROR, "Failed to register superblock pool\n");
            goto rdbfw_alloc_prealloc_err;
        }
        ...
        am->free_pool = rdb_register_um_pool (buf, 1, 0, 
                RDB_KFIFO | RDB_NO_IDX | RDB_BTREE, NULL );
        if ( am->free_pool == NULL) {
            fwlog ( LOG_ERROR, "Failed to register free pool\n");
            goto rdbfw_alloc_prealloc_err;
        }
    ...
    for ( i = 0 ; i < ((add_on==1) ? 1 : am->sb_cnt) ; i++ ) {
        ...
        if (sb == NULL) {
            fwlog ( LOG_ERROR, "Failure to allocate superblock of %d bytes\n", am->sb_gsz );
            if (add_on) {
                am->sb_cnt--;
            }
            // TODO: unwind ?
        ...
        fwlog (LOG_DEBUG,"SB.%d @%p\n", i, sb);
        superblocks_allocated++;
        sb->block_sz = sz;
        sb->am = am;
        sb->data_addr = (void *) sb + offsetof (superblock_t, data);
        if ( 0 == rdb_insert (am->superblock_pool, sb ) ) {
            fwlog (LOG_ERROR, "Unable to insert superblock record\n");
            free ( sb );
            goto rdbfw_alloc_prealloc_err;
        }
        ...
        for ( j = 0 ; j < am->sb_item_cnt ; j++ ) {
            ...
            if ( 0 == rdb_insert (am->free_pool, blk ) ) {
                fwlog (LOG_ERROR, "Unable to insert block record\n");
                goto rdbfw_alloc_prealloc_err;
            }
            fwlog (LOG_DEBUG_MORE,"SBd.%d.%d @%p (data @%p, SB=%p) \n", i, j, blk, blk->data_addr,blk->sb);
        }
    }
...
int rdbfw_up_ref ( void *ptr, int count ) {
    ...
    if (ptr) {
        ...
        fwlog ( LOG_ERROR, "attempt to up-ref a null pointer\n");
        return -1;
    }
    if (count < 1) {
        fwlog ( LOG_ERROR, "attempt to up-ref by an invalid count (%d)\n", count);
        return -1;
    }
...
void rdbfw_free(void *ptr){
    ...
    if ( 0 == tmp_ref_ct ) {
        ...
        fwlog ( LOG_DEBUG_MORE, "(internal) free block of size %u at %p-%p\n", blk->sb->block_sz, blk, ptr);
    ...
    if ( tmp_ref_ct > 0 ) {
        ...
        fwlog ( LOG_FATAL, "double free at fwalloc\n" );
        exit (1);
    }
